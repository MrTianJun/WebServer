...
{
public:
    static const int FILENAME_LEN = 200; // max len of filename
...
public:
    // 多有socket上的事件都被注册到同一个epoll内核事件表中，所以将epoll文件描述符设置为静态的
    static int m_epollfd;
    // 统计用户数量
    static int m_user_count;

private:
    // connection socket and client's address
    int m_sockfd;
